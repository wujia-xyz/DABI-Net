"""
Train SW-ForkNet on BUSI Dataset with 5-fold cross-validation.
...
from model import create_sw_forknet

# Fixed output directory
EXP_DIR = './outputs/BUSI/single_modal/SW_ForkNet'
...
def train_one_fold(fold, train_paths, train_labels, val_paths, val_labels, args, log_file):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if metrics['f1'] > best_f1:
            ...
            torch.save(checkpoint, os.path.join(fold_dir, 'best_f1.pth'))
...
def main():
    ...
    config_payload = {
        'method': {
            ...
            'reference': 'Uzen et al., Expert Systems with Applications, 2024',
        },
        'dataset': {
            'name': 'BUSI',
    ...
    # Training log
    log_file = open(os.path.join(EXP_DIR, 'training.log'), 'w')
    ...
    with open(os.path.join(EXP_DIR, 'summary.txt'), 'w') as f:
        f.write("SW-ForkNet on BUSI Dataset\n")
