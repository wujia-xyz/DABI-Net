...
class DINOv2DABINetV2Model(nn.Module):
    ...
    def __init__(
        ...
        # Load DINOv2 with automatic fallback
        self._load_dinov2(load_method, dinov2_model)
    ...
    def _load_dinov2(self, load_method, dinov2_model):
        """Load DINOv2 backbone with automatic fallback.

        Tries the specified method first. If 'hf' fails (e.g., no internet),
        falls back to local torch hub cache, and vice versa.
        """
        import os
        model_map = {
        ...
        if load_method == 'hf':
            try:
                ...
                print(f"HuggingFace loading failed ({e.__class__.__name__}), trying local torch hub...")
                if os.path.isdir(local_dir):
                    ...
                    return
                raise RuntimeError(
                    "Cannot load DINOv2: HuggingFace unreachable and no local cache found. "
            ...
            if os.path.isdir(local_dir):
                ...
                print("Local torch hub cache not found, trying HuggingFace...")
                from transformers import AutoModel
                self.dinov2 = AutoModel.from_pretrained(
                    model_map.get(dinov2_model, dinov2_model)
                )
