...
from .backbone import SharedBackBoneFPN, SharedTransformer
from .revtorch import ModalTransition


class TDF_Net(nn.Module):
    def __init__(self, num_class=2, depth=-4, pretrain=True, channel=512, backbone='vit'):
        super().__init__()

        # the shared backbone network
        if backbone == 'vit':
            self.shared_backbone = SharedTransformer(pretrain=pretrain, out_channels=channel, out_size=28)
        else:
